...
Last checked from public pages: **2026-07-23**.
...
| GitHub latest release | `https://github.com/ucsahinn/vaultpilot/releases/latest` is expected to redirect to `v3.0.2` once publishing finishes. The release record uses customer-facing VaultPilot notes, the July 23, 2026 release date, released asset sizes, SHA-256 values, release-verification guidance, and support links. | Recheck the redirect and asset metadata after publishing or replacing any release asset. | Owner action is needed only if the public release record drifts from the signed manifest or a newer supported release becomes available. |
