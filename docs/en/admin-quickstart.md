...
| Step | Action | Proof |
| --- | --- | --- |
| 1 | Download `PassMan-1.7.0-x64.msi` from the latest GitHub Release. | File name, file size and release tag match [release assets](release-asset-verification.md). |
| 2 | Verify the signed update manifest before trusting the package. | `passman-update.json` lists version `1.7.0`, MSI hash, size and signer thumbprint. |
