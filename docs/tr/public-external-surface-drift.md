...
Public sayfalarda son kontrol: **2026-07-23**.
...
| GitHub latest release | `https://github.com/ucsahinn/vaultpilot/releases/latest` yayın tamamlandıktan sonra `v3.0.2` tag'ine yönlenmelidir. Release kaydı müşteri odaklı VaultPilot notlarını, 23 Temmuz 2026 yayın tarihini, yayımlanan dosya boyutlarını, SHA-256 değerlerini, doğrulama yönlendirmesini ve destek bağlantılarını içerir. | Yayından veya herhangi bir release dosyası değişikliğinden sonra yönlendirmeyi ve asset metadata'sını yeniden kontrol edin. | Owner aksiyonu yalnız public release kaydı imzalı manifestten saparsa veya daha yeni supported release çıkarsa gerekir. |
