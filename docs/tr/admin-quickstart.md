...
| Adım | Aksiyon | Kanıt |
| --- | --- | --- |
| 1 | Son GitHub Release üzerinden `PassMan-1.7.4-x64.msi` dosyasını indir. | Dosya adı, boyut ve release tag [asset doğrulama](release-asset-verification.md) sayfasıyla uyumlu. |
| 2 | Pakete güvenmeden önce imzalı update manifestini doğrula. | `passman-update.json` içinde `1.7.4`, MSI hash, boyut ve signer thumbprint görünür. |
| 3 | MSI dosyasını elevated Windows oturumundan kur. | `PassMan Server` servisi oluşur ve çalışır. |
| 4 | `http://<SERVER_HOST>:1903` adresini aç. | Sunucu cevap verir; düz HTTP IP erişiminde secure-context uyarısı görünebilir. |
