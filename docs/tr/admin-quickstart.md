...
| 5 | İlk owner profilini HTTPS üzerinden oluştur ve varsayılan kasayı aç. `localhost` yalnızca lokal server doğrulaması için kabul edilebilir. | Users ekranında owner görünür, ilk kasa kullanılabilir. |
...
| Port | Varsayılan public HTTPS portu `1903`, hedef operatörlerden erişilebilir. |
| Tarayıcı | Eklenti için Chromium ailesi; konsol için modern tarayıcı. |
| Yetki | MSI kurulum ve update için Administrator yetkisi. |
| Veri modeli | Lokal VaultPilot data dizini; database bu public repoya konmaz. Legacy PassMan data path yalnızca upgrade alias olarak kalır. |
