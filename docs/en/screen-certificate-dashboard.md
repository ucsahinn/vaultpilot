...
- Separate expired, 30-day renewal-window, revoked or disabled, and missing-date records.
...
The timeline counts non-expired, non-revoked records with readable expiry dates. Each visible bar label covers the following actual range:
...
The status widget assigns every certificate record to one of five classes:
...
**Revoked / disabled** takes precedence in status calculation. A revoked record therefore remains in that class even when its expiry date has passed or is missing.

The **Attention** count is the total of the four non-valid classes. The total-certificate button opens all certificate records; a named status row opens the Certificates screen filtered to that exact status. The **Attention** donut is also clickable, but whenever attention is greater than zero it always opens the **Renewal window** filter. If attention comes only from expired, revoked/disabled, or missing-date records, that result can be empty. Use the named status row for an exact review. When attention is zero, the donut opens the **Valid** filter.

### Certificate distribution

The distribution area groups records in three ways:

- **Origin:** Manual, Imported, AD sync, and Other. **Other** is a summary group without its own source-filter value.
...
- Status rows open the **Valid**, **Renewal window**, **Expired**, **Revoked / disabled**, or **No date** filter.
- Manual, Imported, and AD sync origin rows, plus CA and organization rows, open the matching filtered certificate inventory. The **Other** origin row is excluded from this guarantee.
...
### Review an expired or renewal-window record

1. Select **Expired** or **Renewal window** in the Certificate status widget.
