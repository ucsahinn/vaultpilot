...
VaultPilot 3.0.2 paketi DC Agent `1.2.22` ile gelir; güncel destek durumu için hem servis sarmalayıcısının hem de PowerShell çalışanının `1.2.22` ve `ready` bildirmesi beklenir. Başarılı dizin eşitlemesi tek başına ajanı yükseltmez. Kimliğe bağlı işlem protokolünün alt sınırı `1.2.20` olarak kalır; bu nedenle `1.2.15`–`1.2.19` aralığındaki ajanlar eşitleme yapabilse de kilit açma, parola değişimini zorunlu kılma, rastgele parola atama ve hesabı kapatma işlemlerini güvenli biçimde kapalı tutar. Sürüm 1.2.20 hedefi `objectGUID` ve `objectSid` ile bağlayıp AD tarafından hesaplanan `tokenGroups` ile `primaryGroupID` verisini doğrulamaya başladı; 1.2.22 bu sınırı korurken özel yapılandırma yolu kurtarmasını, idempotent mutasyon sonucu teslimini, belirsiz teslimatın denetimli incelemeye taşınmasını, sertifika güvenini, korunan dizin doğrulamasını ve sınırlı tanılama kaydını güçlendirir. Eksik veya değişmiş kimlik kanıtı hassas işlemleri fail-closed tutar.
...
Hazır durumdaki `1.2.20` veya daha yeni ajan şu kimliğe bağlı yetenekleri bildirebilir: kilidi açma, bir sonraki girişte parola değişimini zorunlu kılma, genel parola ilkesine göre rastgele parola atama ve hesabı kapatma. Güncel paket, ek kurtarma, güven ve sonuç teslimi sertleştirmeleri için `1.2.22` sürümünü taşır. Yerleşik hesaplar ile ajanın bind hesabı her durumda korunur. Ayrıcalıklı ancak yerleşik olmayan bir hedef için otomatik rotasyon ayrıca kalıcı ve açık ilke onayı ister; elle başlatılan işlem kendi ikinci onayını korur.
