...
- Port kullanımda mı?
- `C:\ProgramData\VaultPilot\logs` altında installer log var mı?
- Windows servis durumu nedir?

## Sunucu açılmıyor

1. Sunucuda `https://127.0.0.1:1903/api/profile` kontrol edin.
2. `VaultPilotServer` servis durumunu kontrol edin; yükseltilmiş eski kurulumlarda `PassManServer` legacy alias olarak görünebilir.
3. Firewall ve port yönlendirmesini doğrulayın.
4. Server loglarını redacted olarak inceleyin.

## DC Agent bağlanmıyor

- Agent makinesinden `<VAULTPILOT_URL>` erişimini test edin.
